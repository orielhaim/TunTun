//! WebSocket client with reconnect + polling fallback.
//!
//! Connection is Ed25519-signed the same way HTTP requests are (headers
//! X-Endpoint-Id / X-Timestamp / X-Endpoint-Signature over the path
//! `/v1/ws` and an empty body).
...
/// Spawn a long-lived task that keeps a WebSocket connected with
/// exponential backoff. If we can't connect for > `fallback_after`,
/// emits `None` on the "connected?" channel so the caller can switch
/// to HTTP polling.
