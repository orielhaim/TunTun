...
use crate::persistent::{AgentIdentity, StatePaths};
...
pub enum Command {
    /// One-shot enrollment: generate key, present enrollment token, persist state.
    Enroll(EnrollArgs),
    /// Long-running mode: read persisted state and start the tunnel.
    Run(RunArgs),
    /// Wipe local state (asks for confirmation).
    Reset(ResetArgs),
}
...
pub fn init_logging(cli: &Cli) {
    let filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("info,tuntun_agent=debug"));
...
pub async fn run_enroll(args: EnrollArgs) -> anyhow::Result<()> {
    ...
    let client = crate::control_client::UnauthedClient::new(args.control_url.clone())?;
    let os = std::env::consts::OS.to_string();
    let agent_version = env!("CARGO_PKG_VERSION").to_string();
    let metadata = crate::system_info::collect_system_metadata(&hostname, &agent_version);
    ...
    let persisted = crate::persistent::PersistedState {
    ...
    // Cache the initial snapshot so `run` boots even if control is down.
    crate::persistent::save_snapshot_cache(&paths, &resp.snapshot)?;
...
pub async fn run_agent(args: RunArgs) -> anyhow::Result<()> {
    ...
    let identity = AgentIdentity::load(&paths).with_context(|| {
        format!(
            "no persisted identity in {}; run `enroll` first",
            paths.dir.display()
        )
    })?;
    let persisted = crate::persistent::PersistedState::load(&paths)?;
    tracing::info!(
        endpoint_id = %identity.endpoint_id_hex(),
