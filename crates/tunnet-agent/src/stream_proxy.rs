...
use tunnet_core::stream::{AcceptedStream, StreamHandler, splice_bidirectional};
...
async fn handle_accepted(accepted: AcceptedStream, routes: &RoutingTable) {
    ...
    if host == tunnet_core::ping::PING_HOST {
        let _ =
            tunnet_core::ping::handle_inbound_ping(&accepted.header, accepted.send, accepted.recv)
                .await;
        return;
    }
