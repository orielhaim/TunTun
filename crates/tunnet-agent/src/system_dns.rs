//! Platform DNS configuration so the OS uses Tunnet PeerDNS.
...
mod linux {
    ...
    pub fn configure(dns_ip: Ipv4Addr, suffix: &str) -> anyhow::Result<DnsGuard> {
        ...
        if resolved_dir.parent().is_some_and(|p| p.exists()) {
            ...
            let conf_path = resolved_dir.join("tunnet-PeerDNS.conf");
        ...
        let backup = PathBuf::from("/etc/resolv.conf.tunnet.bak");
        ...
        let mut new_content = format!("# Tunnet PeerDNS\nnameserver {dns_ip}\n");
...
mod windows {
    ...
    const NRPT_DISPLAY_NAME: &str = "Tunnet PeerDNS";

    pub fn configure(dns_ip: Ipv4Addr, suffix: &str) -> anyhow::Result<DnsGuard> {
        let dns_ip_s = dns_ip.to_string();

        // Interface DNS (used when tunnet0 wins the metric race).
        ...
        match status {
            Ok(s) if s.success() => {
                tracing::info!(%dns_ip, "configured Windows DNS for tunnet0");
            }
        ...
        Ok(DnsGuard {
            _restore: Some(Box::new(move || {
                ...
                let _ = std::process::Command::new("netsh")
                    .args(["interface", "ip", "delete", "dns", "name=tunnet0", "all"])
                    .creation_flags(CREATE_NO_WINDOW)
                    .status();
            })),
        })
    ...
    fn add_nrpt_rule(namespace: &str, dns_ip: &str) -> bool {
        // Replace any prior Tunnet rule so restarts don't accumulate duplicates.
        remove_nrpt_rule();
        let script = format!(
