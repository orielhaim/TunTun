import { createDb, schema } from "@tunnet/db";
...
export async function resolveTestEnv(): Promise<TestEnv | null> {
  ...
  const apiKey = process.env.TUNNET_TEST_SDK_API_KEY;
  if (!databaseUrl || !apiKey) {
    return null;
  }

  const managementUrl =
    process.env.TUNNET_TEST_MANAGEMENT_URL ??
    process.env.MANAGEMENT_URL ??
    "http://127.0.0.1:3000";
  const controlUrl =
    process.env.TUNNET_TEST_CONTROL_URL ??
    process.env.CONTROL_PLANE_URL ??
    "http://127.0.0.1:8080";

  const orgOverride = process.env.TUNNET_TEST_ORG_ID;
  const networkOverride = process.env.TUNNET_TEST_NETWORK_ID;
