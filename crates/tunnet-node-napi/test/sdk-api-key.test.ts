...
if (!env) {
  console.warn(
    "Skipping SDK tests: set DATABASE_URL and TUNNET_TEST_SDK_API_KEY in .env",
  );
} else if (!servicesReady) {
  console.warn(
    "Skipping SDK tests: start tunnet-control and the management API, then re-run.",
  );
}
...
describe.skipIf(!runTests)("Tunnet SDK native", () => {
  let testEnv: TestEnv;
  let sdk: typeof import("../ts/index.ts") | null = null;
  let ipPrefix: string;
  ...
  beforeAll(async () => {
    ...
    try {
      sdk = await import("../ts/index.ts");
    } catch (error) {
      throw new Error(
        `Failed to load @tunnet/sdk. Run: bun run build:debug --filter @tunnet/sdk\n${String(error)}`,
      );
    }
  });

  test("enrolls via TunnetNode.create with an API key", async () => {
    if (!sdk) throw new Error("SDK not loaded");

    const stateDir = await mkdtemp(join(tmpdir(), "tunnet-sdk-create-"));
    try {
      const node = await sdk.TunnetNode.create({
  ...
  test("reuses persisted identity on subsequent create calls", async () => {
    ...
    const stateDir = await mkdtemp(join(tmpdir(), "tunnet-sdk-reuse-"));
    try {
      const first = await sdk.TunnetNode.create({
        controlUrl: testEnv.controlUrl,
        managementUrl: testEnv.managementUrl,
        apiKey: testEnv.apiKey,
        organizationId: testEnv.orgId,
      ...
      const second = await sdk.TunnetNode.create({
        stateDir,
        standalone: true,
      });
  ...
  test("enrolls explicitly via enroll()", async () => {
    ...
    const stateDir = await mkdtemp(join(tmpdir(), "tunnet-sdk-enroll-"));
    try {
